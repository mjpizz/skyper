...
ApiBridge.prototype.startRepl = function startRepl(options) {
  ...
  this.on("notification", function(body) {
    output.write(color.move(-2, 0));
    output.write(color.yellow.bold("--- " + body));
    output.write("\r\n");
  ...
  this.on("command", function(body) {
    output.write(color.move(-2, 0));
    output.write(color.green.bold(">>> " + JSON.stringify(body)));
    output.write("\r\n");
    replInstance.displayPrompt();
  });
  ...
  this.on("reply", function(body) {
    output.write(color.move(-2, 0));
    output.write(color.magenta.bold("<<< " + JSON.stringify(body)))
    output.write("\r\n");
    replInstance.displayPrompt();
  });
...
function on() {
  ...
  return apiInstance.on.apply(apiInstance, arguments);
}

function send() {
  apiInstance = apiInstance || new ApiBridge();
  return apiInstance.send.apply(apiInstance, arguments);
}

function startRepl() {
  apiInstance = apiInstance || new ApiBridge();
  return apiInstance.startRepl.apply(apiInstance, arguments);
}
