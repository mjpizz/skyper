...
function Bridge() {
  ...
  var python = this._virtualenv.python.bind(this._virtualenv);
  if (process.platform.match(/darwin/i)) {
    python = this._virtualenv.python32bitOSX.bind(this._virtualenv);
  }
  this._child = python([path.join(__dirname, "api.py"), SKYPE_PROTOCOL_VERSION]);
...
function on() {
  apiInstance = apiInstance || new Bridge();
