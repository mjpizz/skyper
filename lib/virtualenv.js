...
VirtualEnv.prototype.create = function create(callback) {
  ...
  var createProc = spawn("python", ["virtualenv.py", path.basename(this._virtualenvPath)], {cwd: this._virtualenvRoot});
...
VirtualEnv.prototype.install = function install(callback) {
  ...
  var pipProc = spawn("bin/pip", ["install"].concat(this._pythonDeps), {cwd: this._virtualenvPath});
